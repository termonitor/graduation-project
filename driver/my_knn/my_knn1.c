...
static int my_knn_read(struct file *filp, char *buffer, size_t length, loff_t * offset)
{
	...
	if(length==-1)
	{
		for(i=0; i<MY_KNN_REG_NUM*4; i++)
		{
			*(buffer+i)=(char)ioread8(Knn_Regs+i);
		}
		bytes_read=i;
	} else if(length>=0 && length<MY_KNN_REG_NUM)
	{
		*(buffer)=(char)ioread8(Knn_Regs+length*4);
		*(buffer+1)=(char)ioread8(Knn_Regs+length*4+1);
		*(buffer+2)=(char)ioread8(Knn_Regs+length*4+2);
		*(buffer+3)=(char)ioread8(Knn_Regs+length*4+3);
		bytes_read=length;
	}
	//返回-1表示错误
	return bytes_read;
