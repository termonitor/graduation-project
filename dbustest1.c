...
int create_paired_device(DBusGProxy *proxy)
{
	...
	while(!dbus_g_proxy_call(proxy, method, &error,
						  G_TYPE_STRING, device_address,
						  DBUS_TYPE_G_OBJECT_PATH, "/xiaoming/agent",
...
int main(int argc, char *argv[])
{
	...
	//首先通过org.bluez.Manager中的getDefaultAdapter方法获取蓝牙适配器的Adapter对象
	/* get proxy */
...
//	send_recv_objectpath_array(proxy);
	...
	//然后通过adapter的SetProperties方法设置Power属性为true
	...
	//调用org.bluez.Adapter的StartDiscovery方法设置开始搜索
...
//	adapt_powered(adapt, FALSE);
	...
	//create_device如字面意思，建立设备，操作后产生/org/bluez/478/hci0/dev_9C_B7_0D_90_EC_52，Paired属性应该为false(还没确认)
/*	DBusGObjectPath *device_path = create_device(adapt);
	if(device_path != NULL) {
		remove_device(adapt, device_path);
	}*/
	//这里create_paired_device 是对蓝牙耳机进行pair操作，操作后产生/org/bluez/478/hci0/dev_9C_B7_0D_90_EC_52，且Paired属性为true
